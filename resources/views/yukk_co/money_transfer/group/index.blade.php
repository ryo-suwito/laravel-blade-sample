<x-app-layout>
    <x-page.header :title="__('cms.Provider Group')">
        <x-slot name="breadcrumb">
            <x-breadcrumb.home />
            <x-breadcrumb.active>{{ __("cms.Provider Group") }}</x-breadcrumb.active>
        </x-slot>
    </x-page.header>

    <x-page.content :title="__('cms.Provider Group')">
        <form action="{{ route('money_transfer.transactions.groups.index') }}" id="form">

            <div class="row mb-3">
                <div class="col d-md-flex d-lg-flex" style="gap: 5px;">
                    <div class="input-group" style="max-width: 400px;">
                        <select name="search_by" id="search_by" class="form-control">
                            <option value="code" @if ($filters['search_by'] == 'code') selected @endif>Group ID</option>
                            <option value="top_up_code"  @if ($filters['search_by'] == 'top_up_code') selected @endif>Top Up ID</option>
                        </select>
                        <div class="btn-group">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>{{ __("cms.Group ID") }}</th>
                            <th>{{ __("cms.Top Up ID") }}</th>
                            <th>{{ __("cms.Tag") }}</th>
                            <th>{{ __("cms.Amount") }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $group['code'] }}</td>
                            <td>{{ $group['deposit']['code'] ?? '' }}</td>
                            <td>{{ $group['transaction']['entity_type'] ?? ''}}</td>
                            <td>{{ number_format($group['amount'], 0, ',', '.') }}</td>
                            <td>{{ number_format($group['fee'], 0, ',', '.') }}</td>
                            <td>{{ number_format($group['total'], 0, ',', '.') }}</td>
                            <td>{{ $group['transaction']['entity']['name'] ?? '' }}</td>
                            <td>{{ date_format(date_create($group['created_at']), 'd-m-Y H:i:s') }}</td>
                            <td>{{ date_format(date_create($group['updated_at']), 'd-m-Y H:i:s') }}</td>
                            <td>
                                <span class="badge
                                ...
                                <div class="list-icons">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a href="{{ route('money_transfer.transactions.groups.show', ['code' => $group['code']]) }}" class="dropdown-item">
    ...
    <script>
        ...
        $('#btn-apply-filter').on('click', function(e) {
            if (!($('#date_filter').is(':checked') || $('#status').is(':checked') || $('#tags').is(':checked'))) {
                swal('error', 'There is no filter checked.');
            ...
            if ($('#status').is(':checked') && $('.status-cb:checked').length == 0) {
                swal('error', 'Choose at least one status');

                return; 
            }

            if ($('#tags').is(':checked') && $('.tag-cb:checked').length == 0) {
    ...
    <script>
        $('input[name="end_date"]').on('change', function(e) {
            ...
            if (startDateEl.val() == '') {
                swal('error', 'Choose start date first');
