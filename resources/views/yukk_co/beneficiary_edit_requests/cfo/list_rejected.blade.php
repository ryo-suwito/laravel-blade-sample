    ...
    <div class="card">
        ...
        <div class="card-body">
            <ul class="nav nav-pills">
                <li class="nav-item"><a href="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo") }}" class="nav-link">@lang("cms.Waiting List")</a></li>
                <li class="nav-item"><a href="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo_approved") }}" class="nav-link">@lang("cms.Approved List")</a></li>
                <li class="nav-item"><a href="#" class="nav-link active">@lang("cms.Rejected List")</a></li>
            </ul>


            <form action="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo_rejected") }}" method="get">
                <div class="row">
            ...
            <table class="table table-bordered table-striped dataTable">
                ...
                <tr>
                    ...
                    <th>@lang("cms.Reject Remark COO")</th>
                    <th>@lang("cms.Reject COO by")</th>
                    <th>@lang("cms.Reject Remark CFO")</th>
                    <th>@lang("cms.Reject CFO by")</th>
                </tr>
                </thead>
                ...
                @foreach($beneficiary_edit_request_list as $index => $beneficiary_edit_request)
                    <tr>
                        ...
                        <td>{{ @$beneficiary_edit_request->rejected_coo_remark }}</td>
                        <td>{{ @$beneficiary_edit_request->rejector_coo_email }}</td>
                        <td>{{ @$beneficiary_edit_request->rejected_cfo_remark }}</td>
                        <td>{{ @$beneficiary_edit_request->rejector_cfo_email }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <ul class="pagination pagination-flat justify-content-end">
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo_approved", array_merge(request()->all(), ["page" => $current_page-1])) }}" class="page-link"><i class="icon-arrow-left12"></i></a>
                            </li>
                        @endif
                        ...
                        @for ($i = max(1, $current_page - $plus_minus_range); $i <= min($current_page + $plus_minus_range, $last_page); $i++)
                            ...
                            @else
                                <li class="page-item">
                                    <a href="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo_approved", array_merge(request()->all(), ["page" => $i])) }}" class="page-link">{{ $i }}</a>
                                </li>
                            @endif
                        @endfor
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.beneficiary_edit_request.list_cfo_approved", array_merge(request()->all(), ["page" => $current_page+1])) }}" class="page-link">
