...
@section('content')
                ...
                <div class="card-body">
                    <div class="row" style="margin-bottom: 32px;">
                        <div class="col-sm-12 text-right">
                            @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("BENEFICIARY_EDIT_REQUEST.EDIT_COO", "AND"))
                                @if (@$beneficiary_edit_request->status == "PENDING")
                                    <a href="{{ route("cms.yukk_co.beneficiary_edit_request.edit_coo", @$beneficiary_edit_request->id) }}" class="btn btn-primary">@lang("cms.Edit")</a>
                                @endif
                            @endif
                            @if (@$beneficiary_edit_request->status == "PENDING")
                                @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("BENEFICIARY_EDIT_REQUEST.APPROVE_COO", "AND"))
                                    <button type="button" class="btn btn-success btn-approve-coo">@lang("cms.Approve")</button>
                                @endif
                                @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("BENEFICIARY_EDIT_REQUEST.REJECT_COO", "AND"))
                                    <button type="button" class="btn btn-danger btn-reject-coo">@lang("cms.Reject")</button>
                                @endif
                            @endif
                        </div>
                    </div>
    ...
    @if (@$beneficiary_edit_request->status == "PENDING")
        @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("BENEFICIARY_EDIT_REQUEST.APPROVE_COO", "AND"))
            <form method="post" id="approve-coo-form" class="hide" style="flex: none;" action="{{ route("cms.yukk_co.beneficiary_edit_request.approve_coo", @$beneficiary_edit_request->id) }}">
                @csrf
            </form>
        @endif

        @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("BENEFICIARY_EDIT_REQUEST.REJECT_COO", "AND"))
            <div id="reject-coo-modal" class="modal fade" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        ...
                        <form method="post" action="{{ route("cms.yukk_co.beneficiary_edit_request.reject_coo", @$beneficiary_edit_request->id) }}" class="form-horizontal">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-danger btn-reject-submit-coo">@lang("cms.Reject")</button>
                            </div>
                        </form>
                    </div>
                </div>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(".btn-approve-coo").click(function(e) {
                if (window.confirm("Apakah anda yakin ingin melakukan perubahan status ini ?")) {
                    $("#approve-coo-form").submit();
                } else {
                    e.preventDefault();
                }
            });

            $(".btn-reject-submit-coo").click(function(e) {
            ...
            $(".btn-reject-coo").click(function(e) {
                $("#reject-coo-modal").modal();
            });
        });
    </script>
@endsection