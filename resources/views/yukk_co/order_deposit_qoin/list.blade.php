...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("cms.yukk_co.order_deposit_qoin.list") }}" method="get">
                <div class="row">
                    <div class="col-lg-6">
            ...
            <table class="table table-bordered table-striped dataTable">
                ...
                @foreach($order_deposit_list as $index => $order_deposit)
                    <tr>
                        <td>{{ @$order_deposit->qoin_order_id }}</td>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <ul class="pagination pagination-flat justify-content-end">
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.order_deposit_qoin.list", array_merge(request()->all(), ["page" => $current_page-1])) }}" class="page-link"><i class="icon-arrow-left12"></i></a>
                            </li>
                        @endif
                        ...
                        @for ($i = max(1, $current_page - $plus_minus_range); $i <= min($current_page + $plus_minus_range, $last_page); $i++)
                            ...
                            @else
                                <li class="page-item">
                                    <a href="{{ route("cms.yukk_co.order_deposit_qoin.list", array_merge(request()->all(), ["page" => $i])) }}" class="page-link">{{ $i }}</a>
                                </li>
                            @endif
                        @endfor
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.order_deposit_qoin.list", array_merge(request()->all(), ["page" => $current_page+1])) }}" class="page-link">
