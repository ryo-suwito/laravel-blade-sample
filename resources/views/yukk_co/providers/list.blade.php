        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-sm-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">@lang("cms.Provider")</span>
                </div>
        ...
        <div class="card-body">
            <table class="table table-bordered table-striped dataTable">
                ...
                @foreach($provider_list as $index => $provider)
                    <tr>
                        <td>{{ @$provider->name }}</td>
                        <td>@if(@$provider->active) @lang("cms.Active") @else @lang("cms.Inactive") @endif</td>
                        <td class="text-center">
                            <div class="list-icons">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{ route("cms.yukk_co.provider.item", @$provider->id) }}" class="dropdown-item"><i class="icon-search4"></i> @lang("cms.Detail")</a>
                                        @if (\App\Helpers\AccessControlHelper::checkCurrentAccessControl("PROVIDER_UPDATE", "AND"))
                                            <a href="{{ route("cms.yukk_co.provider.edit", @$provider->id) }}" class="dropdown-item"><i class="icon-pencil7"></i> @lang("cms.Edit")</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
