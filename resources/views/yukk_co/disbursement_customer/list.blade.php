...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("cms.yukk_co.disbursement_customer.list") }}" method="get">
                <div class="row">
            ...
            <div class="row">
                <div class="form-group col-auto ml-auto">
                    <select type="text" id="per_page" name="per_page" class="form-control" placeholder="@lang("cms.Per Page")" value="{{ @$per_page }}">
            ...
            <table class="table table-bordered table-striped dataTable">
                ...
                @foreach($disbursement_customer_list as $index => $disbursement_customer)
                    <tr>
                        ...
                        <td>{{ @$disbursement_customer->partner->name }}</td>
                        ...
                        <td>{{ @$disbursement_customer->transfer_using }}</td>
                        <td>{{ @$disbursement_customer->bank_type }}</td>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{ route("cms.yukk_co.disbursement_customer.item", @$disbursement_customer->id) }}" class="dropdown-item"><i class="icon-search4"></i> @lang("cms.Detail")</a>
                                    </div>
                                </div>
                            </div>
                        </td>
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <ul class="pagination pagination-flat justify-content-end">
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.disbursement_customer.list", array_merge(request()->all(), ["page" => $current_page-1])) }}" class="page-link"><i class="icon-arrow-left12"></i></a>
                            </li>
                        @endif
                        ...
                        @for ($i = max(1, $current_page - $plus_minus_range); $i <= min($current_page + $plus_minus_range, $last_page); $i++)
                            ...
                            @else
                                <li class="page-item">
                                    <a href="{{ route("cms.yukk_co.disbursement_customer.list", array_merge(request()->all(), ["page" => $i])) }}" class="page-link">{{ $i }}</a>
                                </li>
                            @endif
                        @endfor
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.yukk_co.disbursement_customer.list", array_merge(request()->all(), ["page" => $current_page+1])) }}" class="page-link">
