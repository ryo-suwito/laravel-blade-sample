        ...
        <div class="card-body">
            <div class="row">
                <div class="col-sm-6 alert" style="border: 1px black solid;">
                    <h2>@lang("cms.SUM"): <br/> {{ @\App\Helpers\H::formatNumber(@collect($transaction_pg_list)->sum("grand_total")) }}</h2>
                    <h4>@lang("cms.Jumlah Transaksi"): <br/> {{ @\App\Helpers\H::formatNumber(@collect($transaction_pg_list)->count()) }}</h4>
                    <h4>@lang("cms.Date Range"): <br/> {{ @\App\Helpers\H::formatDateTime($start_time) }} - {{ @\App\Helpers\H::formatDateTime($end_time) }}</h4>
                </div>
            </div>

            <hr>

            <form action="{{ route("cms.merchant_branch.transaction_pg.list") }}" method="get">
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label>@lang("cms.Nominal")</label>
                            <input type="number" name="nominal" class="form-control" placeholder="@lang("cms.Search Nominal")" value="{{ $nominal }}">
                        </div>
                    </div>

                    <div class="col-lg-4">
                        <div class="form-group">
                            <label>@lang("cms.Payment Method")</label>
                    ...
                    <div class="col-lg-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status">
                                ...
                                @foreach (["SUCCESS", "SUCCESS.BELOW_AMOUNT", "WAITING", "PENDING", "FAILED", "CANCELED", "EXPIRED"] as $_status)
                                    <option value="{{ $_status }}" @if($status == $_status) selected @endif>{{ str_replace("_", " ", str_replace(".", " ", $_status)) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label>@lang("cms.Is Settle")</label>
            ...
            <table class="table table-bordered table-striped dataTable">
                ...
                <tr>
                    ...
                    <th>@lang("cms.Need Settlement")</th>
                ...
                @foreach($transaction_pg_list as $index => $transaction_pg)
                    <tr>
                        ...
                        <td>{{ @$transaction_pg->customer_name }}</td>
                        ...
                        <td class="text-right">{{ @\App\Helpers\H::formatNumber($transaction_pg->mdr_external_fixed + $transaction_pg->fee_partner_fixed + $transaction_pg->mdr_external_percentage + $transaction_pg->fee_partner_percentage, 2) }}</td>
                        ...
                        <td>{{ @$transaction_pg->is_settle ? 'Yes' : 'No' }}</td>
                        <td class="text-center">
                            <div class="list-icons">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{ route("cms.merchant_branch.transaction_pg.item", $transaction_pg->id) }}" class="dropdown-item"><i class="icon-search4"></i> @lang("cms.Detail")</a>
                                    </div>
                                </div>
                            </div>
                        </td>
