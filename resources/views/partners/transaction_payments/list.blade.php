...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("cms.partner.transaction_payment.list") }}" method="get">
                <div class="row">
                    ...
                    <div class="col-lg-3">
                        <div class="form-group">
                            ...
                            <input type="text" name="transaction_code" class="form-control" placeholder="@lang("cms.Search RRN")" value="{{ $transaction_code }}">
                        </div>
                    </div>
            ...
            <table class="table table-bordered table-striped dataTable">
                ...
                @foreach($transaction_payment_list as $index => $transaction_payment)
                    <tr>
                        ...
                        <td>{{ $transaction_payment->transaction_code }} ({{ $transaction_payment->qris_qr_type }})</td>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{ route("cms.partner.transaction_payment.show", $transaction_payment->id) }}" class="dropdown-item"><i class="icon-search4"></i> @lang("cms.Detail")</a>
                                    </div>
                                </div>
                            </div>
                        </td>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <ul class="pagination pagination-flat justify-content-end">
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.partner.transaction_payment.list", array_merge(request()->all(), ["page" => $current_page-1])) }}" class="page-link"><i class="icon-arrow-left12"></i></a>
                            </li>
                        @endif
                        ...
                        @for ($i = max(1, $current_page - $plus_minus_range); $i <= min($current_page + $plus_minus_range, $last_page); $i++)
                            ...
                            @else
                                <li class="page-item">
                                    <a href="{{ route("cms.partner.transaction_payment.list", array_merge(request()->all(), ["page" => $i])) }}" class="page-link">{{ $i }}</a>
                                </li>
                            @endif
                        @endfor
                        ...
                        @else
                            <li class="page-item">
                                <a href="{{ route("cms.partner.transaction_payment.list", array_merge(request()->all(), ["page" => $current_page+1])) }}" class="page-link">
