...
class OrderDepositQoinController extends BaseController {


    public function index(Request $request) {
        $access_control = "ORDER_DEPOSIT_QOIN";
        if (AccessControlHelper::checkCurrentAccessControl($access_control, "AND")) {
            ...
            $order_deposit_qoin_response = ApiHelper::requestGeneral("POST", ApiHelper::END_POINT_QOIN_ORDER_DEPOSIT_LIST_YUKK_CO, [
                "query" => $query_params,
            ]);
            //dd($order_deposit_qoin_response);

            if ($order_deposit_qoin_response->is_ok) {
                $result = $order_deposit_qoin_response->result;
                ...
                if ($request->has("export_to_csv")) {
                    $headers = array(
                        ...
                        "Content-Disposition" => "attachment; filename=Report Order deposit QOIN " . $start_time->format("d/m/Y") . " - " . $end_time->format("d/m/Y") . ".csv",
                        "Pragma" => "no-cache",
                    ...
                    return view("yukk_co.order_deposit_qoin.list", [
                        ...
                        "end_time" => $end_time,
                        "status" => $status,
                        "sum_raw_amount" => $sum_raw_amount,
                        "sum_total_amount" => $sum_total_amount,
                    ]);
                }
            } else {
                return $this->getApiResponseNotOkDefaultResponse($order_deposit_qoin_response);
            }
