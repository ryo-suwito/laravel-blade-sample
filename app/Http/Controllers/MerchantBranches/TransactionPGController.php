...
class TransactionPGController extends BaseController {
    ...
    public function index(Request $request) {
        ...
        $nominal = $request->get("nominal", null);
        ...
        if ($nominal !== null) {
            $query_params["nominal"] = $nominal;
        }
        if ($status == "ALL") {
            $query_params["status_list"] = ["SUCCESS", "SUCCESS.BELOW_AMOUNT", "WAITING", "PENDING", "FAILED", "CANCELED", "EXPIRED"];
        } else {
            $query_params["status_list"] = [$status];
        }
        ...
        $payment_channel_response = ApiHelper::requestGeneral("POST", ApiHelper::END_POINT_PG_SERVICE_PAYMENT_CHANNEL_LIST_MERCHANT_BRANCH, [
            "query" => [
                "per_page" => 10000,
            ],
        ]);
        ...
        $transaction_pg_response = ApiHelper::requestGeneral("POST", ApiHelper::END_POINT_ORDER_TRANSACTION_PG_LIST_MERCHANT_BRANCH, [
            "query" => $query_params,
        ]);
        ...
        if ($transaction_pg_response->is_ok) {
            ...
            if ($request->has("export_to_csv")) {
                ...
                $callback = function() use ($transaction_pg_list, $columns)
                {
                    ...
                    foreach($transaction_pg_list as $transaction_pg) {
                        fputcsv($file, [
                            ...
                            @number_format(($transaction_pg->mdr_external_fixed + $transaction_pg->fee_partner_fixed + $transaction_pg->mdr_external_percentage + $transaction_pg->fee_partner_percentage), 2, ".", ""),
                            @number_format($transaction_pg->grand_total - ($transaction_pg->mdr_external_fixed + $transaction_pg->fee_partner_fixed + $transaction_pg->mdr_external_percentage + $transaction_pg->fee_partner_percentage), 2, ".", ""),
                ...
                //dd($transaction_pg_response);
                return view("merchant_branches.transaction_pg.list", [
    ...
    public function show(Request $request, $transaction_pg_id) {
        $transaction_payment_response = ApiHelper::requestGeneral("POST", ApiHelper::END_POINT_ORDER_TRANSACTION_PG_ITEM_MERCHANT_BRANCH, [
            "form_params" => [
        ...
        if ($transaction_payment_response->is_ok) {
            ...
            return view("merchant_branches.transaction_pg.show", [
