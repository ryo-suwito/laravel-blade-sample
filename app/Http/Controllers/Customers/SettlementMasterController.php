...
class SettlementMasterController extends BaseController {
    ...
    public function downloadExcel(Request $request) {
        ...
        $settlement_master_response = ApiHelper::requestGeneral("POST", ApiHelper::END_POINT_DISBURSEMENT_SETTLEMENT_MASTER_ITEM_CUSTOMER, [
            "form_params" => [
        ...
        $filename = "Transaction_Report_" . Carbon::parse($settlement_master->settlement_date)->format("Y_m_d");
    ...
    public static function generateSpreadSheet($settlement_master) {
        ...
        foreach ($settlement_master->settlement_detail_list as $settlement_detail) {
            ...
            $mdr = @($settlement_detail->transaction_payment->yukk_portion + $settlement_detail->transaction_payment->fee_partner_percentage + $settlement_detail->transaction_payment->fee_partner_fixed + $settlement_detail->transaction_payment->fee_yukk_additional_percentage + $settlement_detail->transaction_payment->fee_yukk_additional_fixed) ?? "";
            $merchant_portion = @$settlement_detail->transaction_payment->merchant_portion ?? "";
